                        ...
                        <div class="card">
                            <div class="card-body">
                                <form action="{{route('roles.update',$role)}}" method="post" >
                                    @csrf
                                    @method('PATCH')
                                    <h1 class="card-title fs-1">فرم ویرایش نقش</h1>
                                    <div class="container">

                                        <h5>عنوان</h5>
                                        <input type="text" id="title" name="title"  class="form-control" value="{{$role_title}}" placeholder="عنوان">
                                        ...
                                        @foreach($permissions as $row)
                                            <!-- Rounded switch -->
                                                <div class="col-md-6">
                                                    <label class="switch">
                                                        <input type="checkbox" @if(in_array($row->id,$permission_ids)) checked @endif name="permissions[]" value="{{$row->id}}">
